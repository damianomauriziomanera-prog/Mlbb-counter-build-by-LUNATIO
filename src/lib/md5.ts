/**
 * High-performance, self-contained MD5 implementation for generating Wikia CDN paths.
 */
import fallbackIcons from '../data/icons.json';
...
function md5(str: string): string {
  ...
  const utf8Joined = Utf8Encode(str);
  x = ConvertToWordArray(utf8Joined);
...
/**
 * Directly formats and returns a working, hotlink-safe static.wikia.nocookie.net CDN path.
 */
export function getWikiaImageUrl(filename: string): string {
  // If the filename is an absolute Unsplash URL or similar, return it directly
  if (filename.startsWith('http://') || filename.startsWith('https://')) {
    return filename;
  }

  // Formatting name: e.g. "Chang'e.png" (must contain raw quotes, not encoded, for calculating MD5 hash used by Wikia filesystem)
  // Let's decode if it comes loaded with %27 or other encodings
  const decodedName = decodeURIComponent(filename);
  
  // Spaces MUST be replaced with underscores for the Wikia filesystem
  const cleanName = decodedName.replace(/\s+/g, '_');
  
  // Use precise pre-fetched URL if available (solves Fandom routing issues for Emblems/Spells)
  if (fallbackIcons && (fallbackIcons as Record<string, string>)[cleanName]) {
    return (fallbackIcons as Record<string, string>)[cleanName];
  }
  
  // Compute MD5 of the filename to find hash directories
  const hash = md5(cleanName);
  const firstChar = hash.charAt(0);
  const twoChars = hash.substring(0, 2);

  // Re-encode specifically for URL query safety but preserve case and punctuation matching the file upload
  const urlEncodedName = encodeURIComponent(cleanName)
    .replace(/%27/g, "'") // Wikia allows single quote in directory lookup name
    .replace(/%28/g, '(')
    .replace(/%29/g, ')');

  return `https://static.wikia.nocookie.net/mobile-legends/images/${firstChar}/${twoChars}/${urlEncodedName}`;
}
