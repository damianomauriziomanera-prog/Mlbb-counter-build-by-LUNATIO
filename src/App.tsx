...
import { simulateLaneWinRate } from './lib/laneSimulator';
...
import { exportBuildToCode, importBuildFromCode } from './lib/buildShare';
...
const BuildSlotCard: React.FC<{ 
  ...
  return (
    <motion.div 
      ...
      className={`relative ${showTooltip ? 'z-50' : 'z-10'}`}
...
function HeroEncyclopediaModal({ isOpen, onClose, inline }: { isOpen: boolean; onClose: () => void; inline?: boolean }) {
  ...
  return (
    <div className={inline ? "w-full animate-fadeIn" : "fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4"}>
      <div className={inline ? "bg-slate-900 border border-slate-850 rounded-2xl w-full flex flex-col shadow-xl overflow-hidden min-h-[70vh]" : "bg-slate-900 border border-slate-700/50 rounded-xl w-full max-w-4xl h-[85vh] flex flex-col shadow-2xl overflow-hidden"}>
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50 min-w-0">
          <h2 className="text-base sm:text-xl font-serif text-amber-500 flex items-center gap-2 truncate">
            <BookOpen size={20} className="shrink-0" />
            <span className="truncate">Enciclopedia Eroi</span>
          </h2>
...
function ItemEncyclopediaModal({ isOpen, onClose, inline }: { isOpen: boolean; onClose: () => void; inline?: boolean }) {
  const [search, setSearch] = useState('');
  ...
  return (
    <div className={inline ? "w-full animate-fadeIn" : "fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4"}>
      <div className={inline ? "bg-slate-900 border border-slate-850 rounded-2xl w-full flex flex-col shadow-xl overflow-hidden min-h-[70vh]" : "bg-slate-900 border border-slate-700/50 rounded-xl w-full max-w-5xl h-[85vh] flex flex-col shadow-2xl overflow-hidden"}>
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50 min-w-0">
          <h2 className="text-base sm:text-xl font-serif text-amber-500 flex items-center gap-2 truncate">
            <Scroll size={20} className="shrink-0" />
            <span className="truncate">Enciclopedia Equipaggiamenti</span>
          </h2>
          <button onClick={onClose} className="p-2 text-slate-400 hover:text-white transition-colors shrink-0">
            <X size={24} />
          </button>
...
function ReportBugModal({ 
  ...
  const [bugType, setBugType] = useState('Problemi di visione delle icone');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fullMessage = `Applicazione: MLBB Counter Builder by LUNATIO
...
${bug}`;
    ...
    try {
      ...
      if (data.success) {
        if (data.mock) {
            alert("⚠️ ATTENZIONE: CONFIGURAZIONE EMAIL MANCANTE ⚠️\n\nL'email NON è stata inviata perché manca la configurazione di sicurezza.\n\nContatta l'amministratore di sistema per completare la configurazione dell'ambiente.");
            onSuccess("Segnalazione registrata ma NON spedita.");
        } else {
            onSuccess("Segnalazione inviata con successo! Grazie per il tuo feedback.");
        }
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700/50 rounded-lg p-6 max-w-md w-full shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Tipo di Problema</label>
            <select 
              value={bugType}
              onChange={(e) => setBugType(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-slate-200 focus:outline-none focus:border-amber-500/50"
            >
              <option value="Problemi di visione delle icone">Problemi di visione delle icone</option>
              <option value="Equipaggiamento sbagliato">Equipaggiamento sbagliato</option>
              <option value="Problemi di grafica / Layout">Problemi di grafica / Layout</option>
              <option value="Errori di build (strategia errata)">Errori di build (strategia errata)</option>
              <option value="Altro">Altro</option>
            </select>
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Tua Email (opzionale)</label>
            <input 
              type="email"
              value={userEmail}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Descrizione</label>
            <textarea 
              value={bug}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button 
              type="submit"
              disabled={isSubmitting}
              className="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded font-medium transition-colors disabled:opacity-50 flex items-center gap-2"
            >
...
function PatchNotesModal({ 
    ...
    setTimeout(() => {
      ...
      setAppPatchVersion(prevAppPatch => {
        ...
        if (parts.length > 0) {
          ...
          safeStorage.setItem('mlbb_app_patch_version', nextVal);
          nextAppPatch = nextVal;
          return nextVal;
        }
        const defaultNext = '1.1.8';
        safeStorage.setItem('mlbb_app_patch_version', defaultNext);
      ...
      setTimeout(() => {
        ...
        safeStorage.setItem('mlbb_app_patch_history', JSON.stringify(updatedHistory));
    ...
    setTimeout(() => {
      ...
      setGamePatchVersion(prevGamePatch => {
        ...
        if (parts.length > 0) {
          ...
          const nextVal = parts.join('.');
          safeStorage.setItem('mlbb_game_patch_version', nextVal);
          nextGamePatch = nextVal;
          return nextVal;
        }
        const defaultNext = '1.8.93';
        safeStorage.setItem('mlbb_game_patch_version', defaultNext);
      ...
      setTimeout(() => {
        ...
        safeStorage.setItem('mlbb_game_patch_history', JSON.stringify(updatedHistory));
  ...
  return (
    <div className={inline ? "w-full animate-fadeIn" : "fixed inset-0 bg-black/85 backdrop-blur-md z-[110] flex items-center justify-center p-4"}>
      <motion.div 
        initial={inline ? { opacity: 0, y: 10 } : { opacity: 0, scale: 0.95, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className={inline ? "bg-slate-900 border border-slate-850 rounded-2xl w-full flex flex-col shadow-xl overflow-hidden min-h-[70vh] text-slate-300" : "bg-slate-900 border border-slate-700/60 rounded-xl w-full max-w-4xl h-[85vh] flex flex-col shadow-2xl overflow-hidden text-slate-300"}
      >
        {/* Header */}
...
function ImageWithFallback({ src, alt, className = '', type, id, title }: ImageWithFallbackProps) {
  ...
  return (
    <img 
      ...
      title={title || alt}
      onError={() => {
        if (!imgSrc.includes('weserv') && !imgSrc.includes('/icons/')) {
          setImgSrc(getProxiedUrl(src));
        } else if (!imgSrc.includes('/icons/')) {
          setImgSrc(`/icons/${id.toLowerCase()}.png`);
        } else if (imgSrc.endsWith('.png')) {
          setImgSrc(`/icons/${id.toLowerCase()}.jpg`);
        } else if (imgSrc.endsWith('.jpg')) {
          setImgSrc(`/icons/${id.toLowerCase()}.jpeg`);
        } else if (imgSrc.endsWith('.jpeg')) {
          setImgSrc(`/icons/${id.toLowerCase()}.webp`);
        } else {
          setHasError(true);
        }
      }} 
...
export default function App() {
  ...
  const [builderMobileTab, setBuilderMobileTab] = useState('config');
  const [isGuideVisible, setIsGuideVisible] = useState(true);

  // Synchronizers to match legacy popup behaviors seamlessly
  const handleSetTab = (tab) => {
    setCurrentTab(tab);
    setIsMetaOpen(tab === 'meta');
    setIsHeroEncyclopediaOpen(tab === 'heroes');
    setIsItemEncyclopediaOpen(tab === 'items');
    setIsSavedBuildsOpen(tab === 'saved');
    setIsPatchModalOpen(tab === 'changelog');
  };

  const handleCloseTab = () => {
    setCurrentTab('builder');
    setIsMetaOpen(false);
    setIsHeroEncyclopediaOpen(false);
    setIsItemEncyclopediaOpen(false);
    setIsSavedBuildsOpen(false);
    setIsPatchModalOpen(false);
  };

  useEffect(() => {
    if (isMetaOpen) setCurrentTab('meta');
  }, [isMetaOpen]);
  ...
  useEffect(() => {
    ...
    // Intercetta l'evento nativo di Android/Chrome
  ...
  useEffect(() => {
    if (isHeroEncyclopediaOpen) setCurrentTab('heroes');
  }, [isHeroEncyclopediaOpen]);

  useEffect(() => {
    if (isItemEncyclopediaOpen) setCurrentTab('items');
  }, [isItemEncyclopediaOpen]);

  useEffect(() => {
    if (isSavedBuildsOpen) setCurrentTab('saved');
  }, [isSavedBuildsOpen]);
  ...
  useEffect(() => {
    if (isPatchModalOpen) setCurrentTab('changelog');
  }, [isPatchModalOpen]);
  const [liveMeta, setLiveMeta] = useState<MetaHero[]>(() => {
    const saved = safeStorage.getItem('mlbb_live_meta_updated');
    if (saved) {
  ...
  const [appPatchVersion, setAppPatchVersion] = useState<string>(() => {
    return safeStorage.getItem('mlbb_app_patch_version') || appChangelog[0].version;
  });

  const [appPatchHistory, setAppPatchHistory] = useState<any[]>(() => {
    const saved = safeStorage.getItem('mlbb_app_patch_history');
    if (saved) {
  ...
  const [gamePatchHistory, setGamePatchHistory] = useState<any[]>(() => {
    const saved = safeStorage.getItem('mlbb_game_patch_history');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { /* ignore */ }
    }
    return [
  ...
  const [lastSyncTime, setLastSyncTime] = useState<string>(() => {
    const saved = safeStorage.getItem('mlbb_meta_last_sync_time');
    if (saved) return saved;
    return getTodayFormattedTime();
  });

  React.useEffect(() => {
    const savedTime = safeStorage.getItem('mlbb_meta_last_sync_time');
    if (!savedTime) {
      const todayTime = getTodayFormattedTime();
      safeStorage.setItem('mlbb_meta_last_sync_time', todayTime);
    ...
    steps.forEach((step, index) => {
      setTimeout(() => {
        ...
        if (index === steps.length - 1) {
          ...
          safeStorage.setItem('mlbb_live_meta_updated', JSON.stringify(updatedMeta));
          ...
          safeStorage.setItem('mlbb_meta_last_sync_time', formattedDate);
  ...
  const saveBuild = () => {
    ...
    safeStorage.setItem('mlbb_saved_builds', JSON.stringify(updated));
...
Generato e simulato su: ${window.location.href}
  ...
  const powerSpikes = useMemo(() => {
    return userHero ? calculatePowerSpike(userHero, enemies) : null;
  }, [userHero, enemies]);
  ...
  const laneWinRates = useMemo(() => {
    ...
    return lanes.map(l => simulateLaneWinRate(userHero, l, allies, enemies, liveMeta));
  }, [userHero, allies, enemies, liveMeta]);
  ...
  const applyPresetProfile = (presetType: 'crit' | 'penetration' | 'attack_speed' | 'cdr') => {
    ...
    if (presetType === 'crit') {
      ...
      const setTalent = (tier: number, targetId: string) => {
        const arr = (GLOBAL_TALENTS as any)[`tier${tier}`];
        if (arr) {
          return arr.find((x: Talent) => x.id === targetId);
        }
        return undefined;
      };
  ...
  // Lexical Helper Views for Inline and Modal Rendering
const renderMetaView = (isInline = false) => {
  if (!isMetaOpen && !isInline) return null;
  return (
  ...
  const [importErrorStr, setImportErrorStr] = useState('');

  const handleExportBuild = (build: SavedBuild) => {
    const code = exportBuildToCode(build);
    if (code) {
      navigator.clipboard.writeText(code).then(() => {
        alert("Codice copiato negli appunti! Condividilo con chi vuoi.");
      }).catch(() => {
        alert("Errore nella copia. Codice: " + code);
      });
    }
  };
  ...
  const handleImportBuild = () => {
    ...
    const data = importBuildFromCode(importCodeStr.trim(), Date.now());
    if (!data) {
      setImportErrorStr('Codice non valido o corrotto.');
      return;
    }

    const hero = HEROES.find(h => h.id === data.userHeroId);
    if (!hero) {
      setImportErrorStr('Eroe non trovato nel database.');
      return;
    }
    ...
    alert('Build importata con successo!');
  };

const renderSavedBuildsView = (isInline = false) => {
  if (!isSavedBuildsOpen && !isInline) return null;
  return (
    <div className={isInline ? "w-full animate-fadeIn" : "fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"}>
      <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
            ...
            >
              <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setIsImportModalOpen(true)} className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-lg text-xs font-bold hover:bg-emerald-500 hover:text-white transition-all">
                    <CloudDownload size={14} />
                    <span>Importa Codice</span>
                  </button>
                      ...
                      return (
                         <div key={saved.id} className="bg-slate-950 border border-slate-800 rounded-xl p-4 hover:border-amber-500/30 transition-all flex flex-col md:flex-row items-center gap-4 relative group">
                          ...
                          <div className="flex items-center gap-2 shrink-0 w-full md:w-auto mt-2 md:mt-0">
                            <button 
                              onClick={() => handleExportBuild(saved)}
                              className="p-2 text-sky-400 hover:text-white hover:bg-sky-500/80 bg-sky-500/10 border border-sky-500/20 rounded-lg transition-all"
              ...
              <AnimatePresence>
                {isImportModalOpen && (
                  <motion.div 
                    ...
                    className="absolute inset-0 z-50 bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4 rounded-2xl"
                  >
                    <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-sm flex flex-col gap-4 shadow-2xl">
                      <div className="flex items-center gap-2 text-emerald-400 font-bold mb-2">
                        <CloudDownload size={20} />
                        <h3>Importa Build da Codice</h3>
                      </div>
                      
                      <textarea 
                        className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm font-mono text-slate-300 focus:outline-none focus:border-emerald-500 transition-colors h-24 resize-none"
                        placeholder="Incolla qui il codice LUNATIO-..."
                        value={importCodeStr}
                        onChange={(e) => setImportCodeStr(e.target.value)}
                      />
                      
                      {importErrorStr && <div className="text-red-400 text-xs font-bold">{importErrorStr}</div>}
                      
                      <div className="flex gap-2 mt-2">
                        <button onClick={() => {setIsImportModalOpen(false); setImportErrorStr('');}} className="flex-1 py-2 rounded-lg font-bold text-xs bg-slate-800 text-slate-400 hover:bg-slate-700">Annulla</button>
                        <button onClick={handleImportBuild} className="flex-1 py-2 rounded-lg font-bold text-xs bg-emerald-600 text-white hover:bg-emerald-500">Importa</button>
                      </div>
                    </div>
                  </motion.div>
                )}
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 font-sans flex flex-col lg:flex-row pb-20 lg:pb-0">
      
      {/* 💻 PERSISTENT DESKTOP SIDEBAR */}
      <aside className="hidden lg:flex flex-col w-64 border-r border-slate-800 bg-slate-900 shrink-0 p-6 sticky top-0 h-screen overflow-y-auto z-40 select-none">
        <div className="mb-8">
          <h2 className="text-xl font-bold font-serif text-amber-500 tracking-wider">MLBB LUNATIO</h2>
          <p className="text-[10px] text-slate-500 uppercase tracking-widest font-mono font-semibold">Counter Builder Pro</p>
        </div>
        
        <nav className="space-y-2 flex-1">
          <button 
            type="button"
            onClick={() => handleSetTab("builder")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${currentTab === "builder" ? "bg-amber-500/10 text-amber-400 border border-amber-500/30" : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 border border-transparent"}`}
          >
            <Sword size={18} />
          ...
          <button 
            type="button"
            onClick={() => handleSetTab("meta")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer relative ${currentTab === "meta" ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/30" : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 border border-transparent"}`}
          >
            <Activity size={18} />
            <span>Meta Ufficiale</span>
            <span className="flex h-1.5 w-1.5 absolute right-4 top-1/2 -translate-y-1/2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-cyan-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-cyan-500"></span>
            </span>
          </button>
          ...
          <button 
            type="button"
            onClick={() => handleSetTab("heroes")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${currentTab === "heroes" ? "bg-sky-500/10 text-sky-455 border border-sky-500/30" : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 border border-transparent"}`}
          >
            <Users size={18} />
            <span>Enciclopedia Eroi</span>
          </button>
          ...
          >
            <BookOpen size={18} />
            <span>Enciclopedia Oggetti</span>
          </button>

          <button 
            type="button"
            onClick={() => handleSetTab("emblems")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${currentTab === "emblems" ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/30" : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 border border-transparent"}`}
          >
            <Shield size={18} />
            <span>Emblemi & Spells</span>
          </button>

          <button 
            type="button"
            onClick={() => handleSetTab("saved")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer relative ${currentTab === "saved" ? "bg-rose-500/10 text-rose-400 border border-rose-500/30" : "text-slate-400 hover:text-slate-200 hover:bg-slate-805/40 border border-transparent"}`}
          >
            <Heart size={18} />
            <span>Build Salvate</span>
            {savedBuilds.length > 0 && (
              <span className="absolute right-4 top-1/2 -translate-y-1/2 bg-rose-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold">
                {savedBuilds.length}
              </span>
            )}
          </button>
          ...
          <button 
            type="button"
            onClick={() => handleSetTab("changelog")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${currentTab === "changelog" ? "bg-purple-500/10 text-purple-400 border border-purple-500/30" : "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 border border-transparent"}`}
          >
            <List size={18} />
        ...
        <div className="border-t border-slate-800/60 pt-4 mt-auto space-y-3">
          <button 
            ...
            className="w-full flex items-center gap-2 justify-center px-4 py-2 text-xs border border-slate-700/60 rounded-xl hover:bg-slate-800 text-slate-400 font-semibold cursor-pointer"
          >
            <AlertCircle size={14} />
            <span>Segnala Bug</span>
          </button>
          <p className="text-[10px] text-slate-500 text-center font-mono leading-none">&copy; 2026 LUNATIO MLBB</p>
        </div>
      </aside>

      {/* 📱 NATIVE ANDROID/MOBILE BOTTOM NAVIGATION BAR */}
      <div className="lg:hidden fixed bottom-4 left-4 right-4 z-50 bg-slate-900/95 backdrop-blur-md border border-slate-800/80 flex justify-around py-2.5 rounded-2xl shadow-[0_5px_25px_rgba(0,0,0,0.7)] select-none">
        <button 
          onClick={() => handleSetTab("builder")}
          className={`flex flex-col items-center gap-1 flex-1 py-1 px-1 rounded-lg active:scale-95 transition-all text-center ${currentTab === "builder" ? "text-amber-500" : "text-slate-500"}`}
        >
          <Sword size={18} />
          <span className="text-[9px] font-black uppercase tracking-wider font-mono">Builder</span>
        </button>

        <button 
          onClick={() => handleSetTab("meta")}
          className={`flex flex-col items-center gap-1 flex-1 py-1 px-1 rounded-lg active:scale-95 transition-all text-center relative ${currentTab === "meta" ? "text-cyan-400" : "text-slate-500"}`}
        >
          <Activity size={18} />
          <span className="text-[9px] font-black uppercase tracking-wider font-mono">Meta</span>
          <span className="flex h-1.5 w-1.5 absolute top-1 right-5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-cyan-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-cyan-500"></span>
          </span>
        </button>
        ...
        <button 
          onClick={() => handleSetTab("heroes")}
          className={`flex flex-col items-center gap-1 flex-1 py-1 px-1 rounded-lg active:scale-95 transition-all text-center ${currentTab === "heroes" ? "text-sky-400" : "text-slate-400"}`}
        >
          <Users size={18} />
          <span className="text-[9px] font-black uppercase tracking-wider font-mono">Eroi</span>
        </button>
        ...
        >
          <BookOpen size={18} />
          <span className="text-[9px] font-black uppercase tracking-wider font-mono">Oggetti</span>
        </button>

        <button 
          onClick={() => handleSetTab("emblems")}
          className={`flex flex-col items-center gap-1 flex-1 py-1 px-1 rounded-lg active:scale-95 transition-all text-center ${currentTab === "emblems" ? "text-indigo-400" : "text-slate-550"}`}
        >
          <Shield size={18} />
          <span className="text-[9px] font-black uppercase tracking-wider font-mono">Emblemi</span>
        </button>

        <button 
          onClick={() => handleSetTab("saved")}
          className={`flex flex-col items-center gap-1 flex-1 py-1 px-1 rounded-lg active:scale-95 transition-all text-center relative ${currentTab === "saved" ? "text-rose-455" : "text-slate-550"}`}
        >
          <Heart size={18} />
          <span className="text-[9px] font-black uppercase tracking-wider font-mono font-semibold">Salvati</span>
          {savedBuilds.length > 0 && (
            <span className="absolute top-0.5 right-4 bg-rose-500 text-white text-[8px] w-3.5 h-3.5 flex items-center justify-center rounded-full font-bold">
              {savedBuilds.length}
            </span>
          )}
        </button>
        ...
        {currentTab === "builder" && (
          <div className="space-y-6">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-5 gap-4">
              <div>
                <h1 className="text-2xl md:text-3xl font-serif text-amber-500 tracking-wide font-bold bg-slate-900 border border-amber-900/30 px-4 py-2.5 rounded-xl mb-1 flex items-center gap-2 select-none">
                  <Sword size={22} className="text-amber-500 animate-pulse" /> MLBB COUNTER BUILDER
                </h1>
                <p className="text-xs text-slate-500/80 italic flex items-center gap-1.5 select-none pl-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                  Configura l'eroe alleato ed i nemici speculari in tempo reale per vincere la partita
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {(deferredPrompt || isIOS) && (
                  <button
                    ...
                    className="group flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-md transition-all cursor-pointer font-semibold text-xs active:scale-95 font-mono shadow-sm shadow-emerald-500/10"
                    title="Installa App sul Dispositivo"
                  >
                    <CloudDownload size={12} className="group-hover:scale-110 transition-transform duration-300" />
                ...
                <button
                  ...
                  title="Modalità Finestra Fluttuante"
                >
                  <Crosshair size={12} className="group-hover:scale-110 transition-transform duration-300" />
                  <span className="hidden sm:inline">Mini-Mode In-Game</span>
                  <span className="sm:hidden">Mini-Mode</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setPatchModalSubTab("app");
                    handleSetTab("changelog");
                  }}
                  className="group flex items-center gap-1.5 px-2.5 py-1 bg-slate-900 hover:bg-slate-850 hover:text-amber-400 border border-slate-800 hover:border-amber-500/20 rounded-md transition-all cursor-pointer font-semibold text-xs active:scale-95 text-slate-400 font-mono"
                ...
                <button
                  ...
                  className="group flex items-center gap-1.5 px-2.5 py-1 bg-slate-900 hover:bg-slate-850 hover:text-rose-450 border border-slate-800 hover:border-rose-500/20 rounded-md transition-all cursor-pointer font-semibold text-xs active:scale-95 text-slate-400 font-mono"
                ...
                <span className="w-1 h-1 rounded-full bg-slate-800 mx-0.5 hidden sm:block"></span>
                <a
                  href="mailto:bugreport@example.com?subject=Segnalazione Bug MLBB Counter Builder"
                  className="group flex items-center gap-1.5 px-2.5 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 rounded-md transition-all cursor-pointer font-semibold text-xs active:scale-95 font-mono shadow-sm shadow-red-500/10"
                  title="Segnala un Bug o un Problema"
                >
                  <AlertTriangle size={12} className="group-hover:scale-110 transition-transform duration-300" />
            ...
            {/* 📱 MOBILE TAB SELECTOR (Toggles Selections or Verdict on Android to avoid long scrolling) */}
            <div className="flex lg:hidden bg-slate-900/85 p-1 rounded-xl border border-slate-800/80 mb-2 sticky top-1 z-30 backdrop-blur-md shadow-md animate-slideDown">
              <button 
                type="button"
                onClick={() => setBuilderMobileTab("config")}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${builderMobileTab === "config" ? "bg-amber-500 text-slate-950 shadow-md font-extrabold" : "text-slate-400 hover:text-slate-200"}`}
              >
                <Sword size={14} /> Selezioni Draft
              </button>
              <button 
                type="button"
                onClick={() => setBuilderMobileTab("verdict")}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all cursor-pointer relative ${builderMobileTab === "verdict" ? "bg-amber-500 text-slate-950 shadow-md font-extrabold" : "text-slate-400 hover:text-slate-200"}`}
              >
                <Scroll size={14} /> Verdetto Build
                {enemyIds.length > 0 && builderMobileTab === "config" && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className={`lg:col-span-1 space-y-8 ${builderMobileTab === "config" ? "block animate-fadeIn" : "hidden lg:block"}`}>
            {/* Guida Unificata */}
            {isGuideVisible && (
            <motion.div 
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-slate-900 border-l-4 border-amber-500 p-4 rounded-r-lg shadow-lg relative"
            >
              <button 
                onClick={() => setIsGuideVisible(false)}
                className="absolute top-2 right-2 p-1 text-slate-500 hover:text-slate-300 hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
                title="Chiudi guida"
              >
                <X size={14} />
              </button>
              <div className="flex items-start gap-3 pr-4">
                ...
                <div>
                  <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3 leading-none">Guida Rapida per Iniziare</h4>
                  <div className="flex flex-col gap-2 mb-3">
                    <div className="flex items-start gap-2 text-[11px] text-slate-300 leading-tight">
                      <span className="flex items-center justify-center w-4 h-4 rounded-full bg-amber-500 text-slate-900 font-bold text-[9px] shrink-0 mt-0.5">1</span>
                      <p>Scegli il tuo <b>Ruolo (Lane)</b> e il tuo <b>Eroe</b> dal pannello qui sotto.</p>
                    </div>
                    <div className="flex items-start gap-2 text-[11px] text-slate-300 leading-tight">
                      <span className="flex items-center justify-center w-4 h-4 rounded-full bg-amber-500 text-slate-900 font-bold text-[9px] shrink-0 mt-0.5">2</span>
                      <p>Aggiungi gli eroi <b>Nemici</b> che stai affrontando (puoi metterne fino a 5).</p>
                    </div>
                    <div className="flex items-start gap-2 text-[11px] text-slate-300 leading-tight">
                      <span className="flex items-center justify-center w-4 h-4 rounded-full bg-amber-500 text-slate-900 font-bold text-[9px] shrink-0 mt-0.5">3</span>
                      <p>Il sistema calcolerà istantaneamente <b>Oggetti</b>, <b>Emblemi</b> e <b>Incantesimi</b> perfetti per counterarli!</p>
                    </div>
                  </div>
                  <div className="pt-2 border-t border-amber-500/20">
                    <p className="text-[10px] text-amber-200/60 leading-relaxed italic">
                      <span className="text-amber-500 font-bold uppercase mr-1">Pro-Tip Strategico:</span> Compra i counter <b>subito</b> ad inizio partita se il tuo avversario diretto in corsia ha un vantaggio d'oro su di te.
                    </p>
                  </div>
                </div>
              </div>
                          ...
                          {allies.map(h => (
                            <div key={h.id} className="flex items-center gap-1.5 bg-emerald-500/5 text-emerald-400 border border-emerald-500/10 px-2 py-1 rounded text-[11px] font-black uppercase tracking-wider">
                              <HeroImageWithFallback 
                                ...
                                id={h.id}
                                role={h.role} 
                                className="w-5 h-5 rounded-full object-cover border border-emerald-500/20 shrink-0" 
                              />
                              <span>{h.name}</span>
                        ...
                        return (
                          ...
                          >
                            <HeroImageWithFallback 
                              ...
                              id={h.id}
                              role={h.role} 
                              className={`w-5 h-5 rounded-md object-cover border shrink-0 ${
                                status.type === 'meta' ? 'border-amber-500/30' :
                                status.type === 'off_meta' ? 'border-rose-500/30' :
                                'border-cyan-500/30'
                              }`} 
                            />
                            <span className="truncate max-w-[75px] shrink-0">{h.name}</span>
                ...
                <div className="space-y-4">
                  {results && (
                    <>
                      ...
                      <div className="mb-8 p-5 bg-slate-950/60 border border-slate-800 rounded-xl relative overflow-hidden group">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative z-10">
                          ...
                          {[activeTalent1, activeTalent2, activeTalent3].map((talent, i) => {
                            ...
                            // Gather all unique talents for this tier from global pool
                            const availableTalents = GLOBAL_TALENTS[tierKey] || [];
                      ...
                      {/* MACCHINA DEL TEMPO (POWER SPIKES) */}
                      {powerSpikes && userHero && (
                        <div className="mb-8 p-6 bg-slate-950 border border-slate-800 rounded-2xl relative overflow-hidden">
                          ...
                          <div className="flex items-center gap-2 mb-2 relative z-10">
                            ...
                            <h3 className="text-sm font-black uppercase text-white tracking-widest">La Macchina del Tempo (Power Spikes)</h3>
                          </div>
                          <p className="text-[11px] text-slate-400 mb-6 max-w-2xl leading-relaxed relative z-10">
                            {powerSpikes.overallStrategy}
                          </p>

                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
                            {/* Early Game */}
                            <div className={`p-4 rounded-xl border flex flex-col gap-3 transition-colors ${powerSpikes.early > 50 ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-rose-500/10 border-rose-500/30'}`}>
                              <div className="flex justify-between items-center">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Early Game</span>
                                <span className="text-[9px] text-slate-500 font-bold">1 - 8 Min</span>
                              </div>
                              <div className="h-2 w-full bg-slate-900 rounded-full overflow-hidden">
                                <motion.div 
                                  initial={{ width: 0 }}
                                  animate={{ width: `${powerSpikes.early}%` }}
                                  transition={{ duration: 1 }}
                                  className={`h-full ${powerSpikes.early > 50 ? 'bg-emerald-500' : 'bg-rose-500'}`}
                                />
                              </div>
                              <p className={`text-[10px] font-bold ${powerSpikes.early > 50 ? 'text-emerald-400' : 'text-rose-400'}`}>
                                {powerSpikes.earlyMessage}
                              </p>
                            </div>

                            {/* Mid Game */}
                            <div className={`p-4 rounded-xl border flex flex-col gap-3 transition-colors ${powerSpikes.mid > 50 ? 'bg-amber-500/10 border-amber-500/30' : 'bg-slate-800 border-slate-700'}`}>
                              <div className="flex justify-between items-center">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Mid Game</span>
                                <span className="text-[9px] text-slate-500 font-bold">8 - 15 Min</span>
                              </div>
                              <div className="h-2 w-full bg-slate-900 rounded-full overflow-hidden">
                                <motion.div 
                                  initial={{ width: 0 }}
                                  animate={{ width: `${powerSpikes.mid}%` }}
                                  transition={{ duration: 1, delay: 0.2 }}
                                  className={`h-full ${powerSpikes.mid > 50 ? 'bg-amber-500' : 'bg-slate-500'}`}
                                />
                              </div>
                              <p className={`text-[10px] font-bold ${powerSpikes.mid > 50 ? 'text-amber-400' : 'text-slate-400'}`}>
                                {powerSpikes.midMessage}
                              </p>
                            </div>

                            {/* Late Game */}
                            <div className={`p-4 rounded-xl border flex flex-col gap-3 transition-colors ${powerSpikes.late > 50 ? 'bg-purple-500/10 border-purple-500/30' : 'bg-rose-500/10 border-rose-500/30'}`}>
                              <div className="flex justify-between items-center">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Late Game</span>
                                <span className="text-[9px] text-slate-500 font-bold">15+ Min</span>
                              </div>
                              <div className="h-2 w-full bg-slate-900 rounded-full overflow-hidden">
                                <motion.div 
                                  initial={{ width: 0 }}
                                  animate={{ width: `${powerSpikes.late}%` }}
                                  transition={{ duration: 1, delay: 0.4 }}
                                  className={`h-full ${powerSpikes.late > 50 ? 'bg-purple-500' : 'bg-rose-500'}`}
                                />
                              </div>
                              <p className={`text-[10px] font-bold ${powerSpikes.late > 50 ? 'text-purple-400' : 'text-rose-400'}`}>
                                {powerSpikes.lateMessage}
                              </p>
                            </div>
                          </div>
                        </div>
      ...
      {/* Mini Draft Mode Overlay */}
      {isMiniMode && (
        <MiniDraftMode 
          heroes={HEROES}
          enemies={enemies}
          myHero={userHero || null}
          lane={lane}
          onAddEnemy={(id) => setEnemyIds([...enemyIds, id])}
          onRemoveEnemy={(id) => setEnemyIds(enemyIds.filter(e => e !== id))}
          onSetMyHero={setUserHeroId}
          setLane={setLane}
          calculatedBuild={results}
      ...
      {showIOSPrompt && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[250] flex items-center justify-center p-4">
          <motion.div 
            ...
            className="bg-slate-900 border border-emerald-500/30 rounded-2xl max-w-sm w-full p-6 shadow-2xl relative"
          >
            <button onClick={() => setShowIOSPrompt(false)} className="absolute top-4 right-4 text-slate-500 hover:text-white bg-slate-800 rounded p-1">
              <X size={16} />
            </button>
            <div className="flex flex-col items-center text-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 mb-2">
                <CloudDownload size={32} className="text-white" />
              </div>
              <h3 className="text-lg font-black text-white uppercase tracking-widest">Installa su iPhone</h3>
              <p className="text-sm text-slate-300">
                La Apple non permette i download automatici dal web. Per installare l'app e averla sempre a portata di mano:
              </p>
              <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 w-full text-left space-y-3">
                <div className="flex items-center gap-3 text-sm text-slate-300">
                  <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-800 text-amber-500 font-bold shrink-0">1</span>
                  <p>Tocca il tasto <b>Condividi</b> in basso nel tuo browser Safari.</p>
                </div>
                <div className="flex items-center gap-3 text-sm text-slate-300">
                  <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-800 text-amber-500 font-bold shrink-0">2</span>
                  <p>Scorri e seleziona <b className="text-white">"Aggiungi a Schermata Home"</b>.</p>
                </div>
              </div>
              <button 
                onClick={() => setShowIOSPrompt(false)}
                className="w-full py-3 mt-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold uppercase tracking-widest transition-colors"
              >
                Ho capito
              </button>
            </div>
...
function ItemSwapperModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-[120] flex items-center justify-center p-4">
      ...
      >
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-800 flex justify-between items-center bg-slate-950/40 min-w-0">
          <div className="min-w-0 pr-3">
            <h3 className="text-sm sm:text-base font-black text-amber-500 uppercase tracking-widest flex items-center gap-2 truncate">
              <RefreshCw size={16} className="text-amber-500 animate-spin-slow shrink-0" />
              <span className="truncate">Sostituisci Slot {slotIndex + 1}</span>
            </h3>
            <p className="text-[10px] sm:text-xs text-slate-500 font-medium mt-0.5 truncate">Scegli una contromisura alternativa</p>
          </div>
          <button 
            onClick={onClose} 
