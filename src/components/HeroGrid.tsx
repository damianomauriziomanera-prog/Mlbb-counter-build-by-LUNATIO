...
// Sotto-componente ottimizzato: React ricarica la singola carta solo se le sue props (es. isSelected) cambiano
const MemoizedHeroCard = memo(({ 
  h, 
  isSelected, 
  isDisabled, 
  isMeta, 
  ...
  return (
    <motion.button
      ...
      className={`relative group rounded-lg overflow-hidden border-2 transition-all aspect-square min-w-0 min-h-0 shrink-0 ${
        isSelected 
    ...
    >
      <HeroImageWithFallback 
        ...
        id={h.id}
        role={h.role}
        className={`absolute inset-0 w-full h-full object-cover transition-all shrink-0 ${
          isSelected 
            ? 'opacity-100' 
            : isDisabled 
              ? 'opacity-50'
      ...
      {isSelected && (
        <div className={`absolute inset-0 ${colorClasses.bgSoft} flex items-center justify-center bg-slate-950/55`}>
          {<CheckCircle size={20} className={colorClasses.text} />}
        </div>
      )}
  ...
  // Ritorna true se le props sono uguali (non serve ricaricare)
  return (
    prevProps.isSelected === nextProps.isSelected &&
    prevProps.isDisabled === nextProps.isDisabled &&
    prevProps.isMeta === nextProps.isMeta &&
    prevProps.isOffMeta === nextProps.isOffMeta &&
    prevProps.h.id === nextProps.h.id
  );
});
...
export function HeroGrid({ 
  ...
  const colorClassesMap = {
    emerald: { border: 'border-emerald-500', ring: 'ring-emerald-500/10', text: 'text-emerald-500', bgActive: 'bg-emerald-500', bgSoft: 'bg-emerald-500/10', focus: 'focus:border-emerald-500/50 focus:ring-emerald-500/20', icon: 'text-emerald-500' },
    red: { border: 'border-red-500', ring: 'ring-red-500/10', text: 'text-red-500', bgActive: 'bg-red-500', bgSoft: 'bg-red-500/10', focus: 'focus:border-red-500/50 focus:ring-red-500/20', icon: 'text-red-500' },
    amber: { border: 'border-amber-500', ring: 'ring-amber-500/10', text: 'text-amber-500', bgActive: 'bg-amber-500', bgSoft: 'bg-amber-500/10', focus: 'focus:border-amber-500/50 focus:ring-amber-500/20', icon: 'text-amber-500' },
    blue: { border: 'border-blue-500', ring: 'ring-blue-500/10', text: 'text-blue-500', bgActive: 'bg-blue-500', bgSoft: 'bg-blue-500/10', focus: 'focus:border-blue-500/50 focus:ring-blue-500/20', icon: 'text-blue-500' }
  };
    ...
    <div className="space-y-4">
      ...
      <div className={`grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-5 gap-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar border border-slate-800 p-2 rounded bg-slate-950/30`}>
        ...
        {filteredHeroes.map(h => {
          ...
          const isFull = selectedIds.length >= maxSelections && !isSelected && maxSelections > 1;
          const isDisabled = isFull || disabledIds.includes(h.id);

          const metaHero = metaHeroes?.find(mh => mh.id === h.id);
          const isMeta = !!(metaHero && ['S+', 'S'].includes(metaHero.tier));
          const isOffMeta = !!(metaHero && ['B', 'C'].includes(metaHero.tier));
